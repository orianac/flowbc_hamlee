     ...
     char*argv[])
{
    ...
    if (argc != 6) {
        printf(
            "Wrong command line arguments: enter <x time step file> <start year> <end year> <periods per year> <unit switch>,\n");
        exit(0);
    }
...
/* set days per period for July-June split month time series or 12 month water year Oct-Sept*/
    ...
    for (year = startyear; year <= endyear; year++) {
        if ((year % 4 == 0) &&
            ...
            if (timeincpyr == 12) {
                days_per_period[4] = 29;
            }
            if (timeincpyr == 14) {
                days_per_period[8] = 29;
            }
            if (timeincpyr == 52) {
                days_per_period[51] = 9;
            }
        }
        else {
            if (timeincpyr == 12) {
        ...
        for (tinc = 0; tinc < timeincpyr; tinc++) {
            if (fscanf(obsqfile, "%f", &qcfs) == EOF) {
                printf(
                    "end of file reached prematurely check beginning and ending dates and number of periods per year are correct\n");
            }
            ;
