     ...
     char*argv[])
{
    ...
    if (argc != 7) {
        printf(
            "Wrong command line arguments: enter <qfile.obs> <qfile.sim> <inputfile> <outputfile> <dist_opt> <timeperiodsperyr>,\n");
        exit(0);
    }
...
/*read distribution parameters and observed quantile lookup table from file and load to array*/
    ...
    for (i = 0; i < timeincpyr; i++) {
        fscanf(obsqfile, "%f%f%f%f", &uobsmean[i], &uobsstd[i], &obsmean[i],
               &obsstd[i]);
...
/*read simulated quantile lookup table from file and load to array*/
    ...
    for (i = 0; i < timeincpyr; i++) {
        fscanf(simqfile, "%f%f%f%f", &usimmean[i], &usimstd[i], &simmean[i],
               &simstd[i]);
...
/*check to see if flow is higher or lower than any quantile value*/
        if (rawdata == sim[timeinc][0][1]) {
            ...
            ev1quantile =
                exp(-1 *
                    exp(-1 * (rawdata - simzeta[timeinc]) / simalpha[timeinc]));
        ...
        else if (rawdata > sim[timeinc][0][1] ||
                 rawdata < sim[timeinc][numsim - 1][1]) {
            ev1quantile =
                exp(-1 *
                    exp(-1 * (rawdata - simzeta[timeinc]) / simalpha[timeinc]));
            ln3anom = (log(rawdata) - simmean[timeinc]) / simstd[timeinc];
...
/*interpolate*/
                    quantile = sim[timeinc][i][0] +
                               (sim[timeinc][i - 1][0] - sim[timeinc][i][0]) *
                               (rawdata - sim[timeinc][i][1]) /
                               (sim[timeinc][i - 1][1] - sim[timeinc][i][1]);
...
/*check to see if quantile value is out of range of observed lookup table*/
        ...
        else if ((quantile > obs[timeinc][numobs - 1][0]) ||
                 (quantile < obs[timeinc][0][0]) || outrangeflag[0] == 1) {
            if (dist_opt == 0) {
                value = obszeta[timeinc] - obsalpha[timeinc] *
                        log(-1 * log(ev1quantile));
            }
            if (dist_opt == 1) {
                value = exp(obsstd[timeinc] * ln3anom + obsmean[timeinc]) +
                        obsa[timeinc];
            }
        }
        else {
...
/*interpolate*/
                    ...
                    value = obs[timeinc][i][1] +
                            (obs[timeinc][i - 1][1] - obs[timeinc][i][1]) *
                            (obs[timeinc][i][0] - quantile) /
                            (obs[timeinc][i][0] - obs[timeinc][i - 1][0]);
...
/*exit loop*/
        ...
        if (rawdata > -0.001 && rawdata < 0.001) {
            value = 0;
        }
/* if rawdata is sufficiently out of range as defined by preset threshold value, use simple scaling technique */
                 ...
                 (ev1quantile < (1.0 - ev1thresh) || ev1quantile > ev1thresh)) {
            value = rawdata / usimmean[timeinc] * uobsmean[timeinc];
        }
        else if (dist_opt == 1 &&
                 (ln3anom < (-1 * thresh) || ln3anom > thresh)) {
            value = rawdata / usimmean[timeinc] * uobsmean[timeinc];
        }
