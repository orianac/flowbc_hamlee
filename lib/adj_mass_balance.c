...
main(int  argc,
     char*argv[])
{
    FILE *configfile, *rawdownfile, *rawupfile[5], *bcdownfile, *bcupfile[5],
         *outfile[5];
    ...
    int   dpsm[14] = {
        31, 15, 16, 30, 31, 30, 31, 31, 28, 31, 15, 15, 31, 30
    };
    int   dpm[12] = {
    ...
    if (argc != 7) {
        printf(
            "Wrong command line arguments: enter <config_file> <out_dir_path> <start year> <ppy> <unit switch> <file str> ,\n");
        exit(0);
    }
...
// finish loading bc.ff and bc.f data
            ...
            for (i = 0; i < (numstat - 1); i++) {
                for (j = 0; j < ppy; j++) {
                    if (fscanf(bcupfile[i], "%f", &tempdata) != 1) {
                        printf("error reading upfile\n");
                    }
