     ...
     char*argv[])
{
    ...
    if (argc != 5) {
        printf(
            "Wrong command line arguments: enter <qfile.obs> <qfile.sim> <inputfile> <outputfile>,\n");
        exit(0);
    }
...
/* if rawdata is sufficiently out of range as defined by preset threshold value, use simple scaling technique */
        ...
        else if (ln3anom < (-1 * thresh) || ln3anom > thresh) {
...
/* print diagnostic data to the screen and bias corrected value to output file*/
        ...
        printf("%d %f %f %f %f %d \n", timeinc, rawdata, quantile, ln3anom,
               value, outrangeflag[0]);
