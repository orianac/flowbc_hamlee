     ...
     char*argv[])
{
    ...
    if (argc != 7) {
        printf(
            "Wrong command line arguments: enter <bcmfile> <bcyfile> <outfile> <startyear> <endyear> <unit sw> ,\n");
        exit(0);
    }
    ...
    for (year = startwy; year <= endwy; year++) {
        ...
        for (mnth = 0; mnth < 12; mnth++) {
            ...
            for (day = 1; day <= dpm[mnth]; day++) {
                ...
                if (fscanf(bcmfile, "%f", &tmp) != 1) {
                    printf("Bad data or EOF encountered prematurely.\n");
                    exit(0);
                }
                sum2 += tmp;
                bcmdata[(day - 1)] = tmp;
...
/* after loading last day of each mnth, write out the data and reinitialize the sum variable */
                ...
                if (day == dpm[mnth]) {
                    if (sum1 == 0 || sum2 == 0) {
                        ...
                        if (unitsw == 0) {
                            adjfrac = sum1 / (sum2 / dpm[mnth]);
                        }
                        if (unitsw == 1) {
                            adjfrac = (sum1 * 0.50417) / sum2;
                        }
                    }
