     ...
     char*argv[])
{
    ...
    int   tdeom[12] = {
        30, 60, 91, 122, 150, 181, 211, 242, 272, 303, 334, 364
    };
    int   tdeoml[12] = {
        30, 60, 91, 122, 151, 182, 212, 243, 273, 304, 335, 365
    };
    float temp[4] = {
        0, 0, 0, 0
    };
    ...
    if (argc != 5) {
        printf(
            "Wrong command line arguments: enter <bcdfile> <outfile> <startyear> <endyear> ,\n");
        exit(0);
    }
    /*open files*/
    ...
    while (n < DayYear || year <= endwy) {
        ...
        if (leap == 1) {
            if (n == tdeoml[i] + 1) {
                ...
                OneYear[tdeoml[i]] =
                    (3 * OneYear[tdeoml[i]] + OneYear[tdeoml[i] + 1]) / 4;
                OneYear[tdeoml[i] + 1] = (temp + 3 * OneYear[tdeoml[i] + 1]) /
                                         4;
                i++;
            }
        }
            ...
            if (n == tdeom[i] + 1) {
                ...
                OneYear[tdeom[i]] =
                    (3 * OneYear[tdeom[i]] + OneYear[tdeom[i] + 1]) / 4;
