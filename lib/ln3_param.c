...
/* note reverse order of sorting routine and array numbering from 1 to n in the context of Stedinger's selection point*/
  ...
  else a= (min*max - med*med)/(min + max - 2*med);
...
for(k=0;k<clength;k++){
   ...
   out[2]= exp(out[0] + out[1]*zvalue[0]) + a;
   out[3]= exp(out[0] + out[1]*zvalue[1]) + a;
   out[4]= exp(out[0] + out[1]*zvalue[2]) + a;
   out[5]= exp(out[0] + out[1]*zvalue[3]) + a; 
